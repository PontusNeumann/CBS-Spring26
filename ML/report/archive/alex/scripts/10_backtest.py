...
def compute_pre_yes_price_corrected(raw: pd.DataFrame) -> np.ndarray:
    ...
    HF `price` is per-token (each token has its own price), not YES-normalized.
    `pre_trade_price` in the features parquet is just price.shift(1) and gives
    whatever the previous trade's token-price was. This function tracks last
    seen token1 and token2 prices separately, shifts to exclude current trade,
    and reconstructs YES probability as last_t1 (else 1 - last_t2 else 0.5).
...
def compute_cost_and_edge(test: pd.DataFrame, p_hat: np.ndarray) -> dict:
    """For each trade compute cost-per-token and edge = p_hat − cost.

    Uses pre_yes_price_corrected if present (post per-token-price fix);
    falls back to raw pre_trade_price otherwise.
    """
    ...
    cost = np.clip(cost, 0.001, 0.999)
    edge = p_hat - cost
    return {
        "cost": cost,
        "edge": edge,
        "trader_side_wins_yes": trader_side_wins_yes,
    }
...
def main():
    ...
    for name in model_names:
        ...
        ce = compute_cost_and_edge(test, cal)
        model_preds[name] = {
            "p_hat_raw": raw,
            "p_hat_cal": cal,
            "edge": ce["edge"],

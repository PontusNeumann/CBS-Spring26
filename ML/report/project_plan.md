...
| Split | **Market-cohort, narrow-window training (22 Apr revision, v2).** Train / validation / test are defined at the market level, in strict train→val→test chronological order (avoiding the earlier backward val inversion). **Train:** 4 strike markets — Feb 25, 26, 27, and Feb 28 — for ~202k trades (post settlement-filter) in a cohesive 4-day peak-informed-flow window. Feb 28 is the documented asymmetric-information market per Mitts & Ofir 2026 (USD 143M anomaly) and must be in training, not held out. Train mix is 3 NO markets (Feb 25–27) + 1 YES market (Feb 28); Feb 28 alone is ~61% of training trades, by design. **Val:** `Iran x Israel/US conflict ends by March 15?` (event 236884, ~21k trades, NO, trade span Feb 28 → Mar 17). Cross-event-family (no price-arbitrage leakage to strike training), post-training chronologically, and sized for stable early-stopping + isotonic calibration signal. **Test:** all 7 ceasefire markets across events 355299 (5 markets: Apr 8, 10, 12, 15, 18) and 357625 (2 markets: Apr 14-ext, Apr 18-ext), ~13.4k trades post-filter, all NO resolution, trade span Apr 8 → Apr 19. Strict chronology: train end (Feb 28) → val (Mar 15 market) → test (Apr 8+). Rationale: (a) training on the peak-signal window matches documented informed-flow concentration; (b) val and test are both cross-event-family from training, testing real generalisation; (c) all non-strike markets resolve NO, which is not a problem because `bet_correct` is a per-trade target that remains ~50/50 within every market — the model learns trade-correctness patterns regardless of per-market resolution (see `alex/notes/test-cohort-no-bias.md` for the re-evaluation); (d) strike YES markets (e.g. Mar 15 strike) were considered for test as a YES hedge but rejected because they share event-family microstructure + wallet-arbitrage correlation with training. Memorisation risk from market-identifying absolute-scale features is mitigated by the v3 feature-pruning drops plus the `size_x_time_to_settlement` interaction drop (see P0-8 in `data-pipeline-issues.md`). **Expansion path if initial results are weak:** add the week before (Feb 18–24) to training, or pull a Maduro/Biden-pardons cluster from HF for an expanded test cohort with documented-insider-trading named cases (plan §8). See `scripts/14_build_experiment_splits.py`. The earlier trade-timestamp quantile split is fully superseded. |

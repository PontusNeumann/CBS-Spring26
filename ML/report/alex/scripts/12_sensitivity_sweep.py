...
# Reuse pieces from 11_realistic_backtest.py
import sys
...
spec = importlib.util.spec_from_file_location(
    "rb11", ROOT / "scripts" / "11_realistic_backtest.py"
)
rb11 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(rb11)
...
def make_strategy(p_hat, edge, cost, name, time_to_deadline_sec):
    if name == "general_ev":
        return rb11.general_ev_rule(edge)
    if name == "top5pct_edge":
        return rb11.top_k_mask(edge, 0.05)
    if name == "top1pct_phat":
        return rb11.top_k_mask(p_hat, 0.01)
    if name == "home_run":
        return rb11.home_run_rule(edge, cost, time_to_deadline_sec)
...
def main():
    ...
    res_times = rb11.market_resolution_time(markets)
    ...
    test["pre_yes_price_corrected"] = rb11.compute_pre_yes_price_corrected(test_raw)

...
# Realism v2 parameters (added after first pass produced 2,000× ROI):
# COST_FLOOR — caps the per-token cost. At cost=0.001 the math allows 999× payoff
# but real Polymarket execution at extreme prices faces severe slippage, near-zero
# orderbook depth, and oracle-resolution uncertainty. Flooring at 0.05 caps payoff
# at 19× per win — closer to what's actually executable.
# LIQUIDITY_SCALER — assume N copycats also mirror these signals. Effective
# liquidity per copycat = original_trade_usd / N. We model this by multiplying
# the usd_amount cap by LIQUIDITY_SCALER (1/N).
COST_FLOOR = 0.05
LIQUIDITY_SCALER = 0.10  # i.e. 10 simultaneous copycats sharing the available volume

# Resolution time per market (UTC)
...
def parse_deadline(question: str, year=2026):
...
def market_resolution_time(markets: pd.DataFrame) -> dict:
    """Returns market_id -> resolution_timestamp (epoch seconds)."""
    out = {}
...
def compute_pre_yes_price_corrected(raw: pd.DataFrame) -> np.ndarray:
    """Compute the correct pre-trade YES-token price for each trade.

    Background: `price` in HF schema is per-token (each token's own price), not
    YES-normalized. `pre_trade_price` in the features parquet is just price.shift(1)
    which gives whatever the previous trade's token-price was.

    Algorithm: for each market sorted by timestamp, ffill last token1 price and
    last token2 price separately, shift(1) to exclude current trade. Then
    pre_yes = last_t1 if exists, else 1 - last_t2, else 0.5.

    Returns numpy array aligned with the input dataframe's row order (after
    sorting by market_id, timestamp).
    """
    df = raw.copy()
...
def compute_cost_and_edge(
    test: pd.DataFrame, p_hat: np.ndarray, cost_floor: float = COST_FLOOR
):
    """Compute cost-per-token and edge.

    NOTE: Uses `pre_yes_price_corrected` if present in the dataframe (post-fix)
    rather than the raw `pre_trade_price` (which is per-token, not YES-normalized).
    The bug + fix are documented in alex/notes/pressure-test.md (D-029) — it
    shifted the realistic-backtest ROI from ~45× to ~1.05-1.85× under realism.
    """
...
# ---------------------------------------------------------------------------
# Strategy masks (mirror what 10_backtest.py defined)
# ---------------------------------------------------------------------------


def general_ev_rule(edge, edge_min=0.02):
    return edge > edge_min


def home_run_rule(
    edge, cost, time_to_deadline_sec, edge_min=0.20, cost_max=0.30, time_max=6 * 3600
):
    return (edge > edge_min) & (cost < cost_max) & (time_to_deadline_sec < time_max)
...
def main():
    ...
    for m in models:
        ...
        cost, edge = compute_cost_and_edge(test, cal)

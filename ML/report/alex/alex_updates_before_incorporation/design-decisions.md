...
| 2026-04-19 | **Baselines run (logreg L1/L2, RF, IF, naive market)** — naive market is best single-metric baseline (val PR-AUC 0.60); RF catastrophically inverts on test (ROC 0.15), logreg near chance, IF weak. Direction features (`is_buy`, `is_token1`) dominate RF importance → root cause of inversion. | Done |
| 2026-04-19 | **`price` re-added to feature set after baseline diagnosis** — improves RF slightly but doesn't solve inversion because direction × outcome interaction flips between NO-dominant training and YES-dominant test. | Locked (revises earlier "price dropped" decision) |
| 2026-04-19 | **MLP v1 trained on `bet_correct`** — val ROC 0.62 (calibrated), test ROC 0.23 (inverted). Same problem as RF. HPO over 6 configs ({128-64, 128-64-32, 256-128-64} × {d0.2, d0.4}) — all invert on test, architecture is not the fix. | Done |
| 2026-04-19 | **Permutation importance (L14) on MLP v1** — top features: `is_buy` (0.072), `is_token1` (0.067), `market_price_std_1h` (0.058), `time_to_settlement_s` (0.043). Wallet-behavioural features all < 0.01. | Done |
| 2026-04-19 | **Autoencoder arm (L11) trained** — recon error mean: train 0.23, val 0.39, test 0.54 (insider-pressure window genuinely anomalous). Top-decile recon-error × top-decile \|gap\| overlap: val 2.78×, test 1.19× vs null. Signals agree in stable regime, diverge in pressure window. | Done |
| 2026-04-19 | **Reframed MLP — target changed to `market_resolves_yes`, direction features + price excluded** → val ROC on bet_correct jumped 0.62 → 0.94, gap_auc 0.58 → 0.82. Test still weak (ROC 0.50) — dominated by single-market structure (99% Feb 28 trades). No longer inverted, just uncertain. | Done |
| 2026-04-19 | **Severe overfitting** observed in reframed MLP: train loss 0.005, val loss 5.9 (1000× ratio). Root cause: only 7 markets → 7 effective outcome samples. Model memorises per-market trajectory signatures. | Flag for Discussion |
| 2026-04-19 | **Streaming backtest run on reframed MLP predictions** — MLP general +EV returns 78% vs random 93% and naive 92%. MLP home-run has 0% hit rate (179 triggers, all wrong). Model's highest-confidence disagree-with-market bets are the rare 1.3% of test that contradict the dominant YES resolution. | Done — documents an empirical finding, not a fix |
| 2026-04-19 | **Course syllabus coverage complete**: L02 (EDA/preproc), L04 (logreg L1/L2), L05 (RF, PCA in EDA), L07 (class_weight + IF), L08/L09 (MLP with SELU/Glorot/BN/Dropout/Adam/schedule), L11 (autoencoder), L13 (HPO 6-config grid), L14 (permutation importance), L15 (isotonic calibration). | Done |
| 2026-04-19 | Layer 6 Polygonscan enrichment running overnight (3 Etherscan keys × 9 workers, ~7h 16m remaining at 16.5%). Next session: rebuild dataset with Layer 6, retrain both models, re-backtest. | In flight |
| 2026-04-20 | **Enrichment completed**: 55,329 / 55,329 wallets successful (after retry pass), 0 failures | Done |
| 2026-04-20 | **Layer 6 integrated** via `scripts/add_layer6.py` → `iran_strike_labeled_v2.parquet` (451,933 × 75 cols). Bisect-based causal per-trade features confirmed strictly `< t`. Wallet coverage 100%. | Done |
| 2026-04-20 | **Reframed MLP v2 (with Layer 6)**: val target ROC 0.91 → test 0.66 (↑ from 0.55 without Layer 6). Val derived bet_correct ROC 0.94 → test 0.31 (still inverts). Backtest MLP general +EV 77% vs random 93%, home-run −100%. Layer 6 helps target prediction but derived bet_correct still inverts. | Done |
| 2026-04-20 | **Memorisation diagnosis**: train loss collapses to 0.005 at epoch 1 on v2 = model learning market-identifying fingerprints from `time_to_settlement_s`, `market_cumvol_log`, `market_vol_24h_log`, `log_time_to_settlement` etc. With only 7 markets the absolute-scale features let the model lookup "market → outcome" instead of learning generalisable patterns. | Locked finding |
| 2026-04-20 | **v3 fix — dropped market-identifying absolute-scale features** from feature set. Kept `pct_time_elapsed` (already 0-1 within market) and bounded price-state features (`market_price_mean_1h`, `market_price_std_1h`). Train loss epoch 1 rose from 0.08 → 0.36 (memorisation reduced). Val target ROC dropped 0.91 → 0.80 (expected, less overfit). **Test target ROC rose 0.66 → 0.73.** Backtest MLP general +EV rose 77% → **116.4% return, Sharpe 83.5, hit rate 0.549 vs naive 0.524** — beats random by +23.6pp. | Locked |
| 2026-04-20 | **Explicit acknowledgment: still in exploration phase, not final ML pipeline.** The 116% result is a single-seed run on an ad-hoc feature set. Next session: finalise features with documented keep/drop/replace rationale, rerun ALL baselines apples-to-apples on that set, train MLP with 5 seeds for variance estimates, run ablations per feature group, backtest all seeds. | Plan |
| 2026-04-20 | Home-run strategy (edge > 0.20, TTS < 6h, price < 0.30) still −100% across all variants. Consistently selects losing trades. Two interpretations: (a) filter is mis-specified for this test market's dynamics; (b) model's extreme-confidence picks are genuine contrarian signals that happened to be wrong against the specific YES resolution. Defer decision to next session. | Open |

...
6. **External — Polygonscan on-chain identity** ⭐ (promoted to v1 scope 2026-04-19): `wallet_polygon_age_days` (days since first on-chain tx on Polygon), `wallet_nonce` (lifetime on-chain tx count at trade time), `wallet_funded_by_cex` (binary: did the wallet's first non-trivial inbound transfer come from a known centralized-exchange hot wallet — Kraken, Binance, Coinbase?). Computed via `scripts/enrich_polygonscan.py`, run once between dataset build and MLP training. Free API key, ~1 hour for the ~50k unique wallets in our pooled 7-market dataset.
...
**`wallet_polygon_age_days` / `wallet_nonce` / `wallet_funded_by_cex`** — Polygonscan on-chain identity features (promoted to v1 scope 2026-04-19).
- `wallet_polygon_age_days` = days since first on-chain tx on Polygon. Captures wallets that are old on-chain but new to Polymarket (sophisticated crypto users trying prediction markets) vs brand-new on both (textbook insider setup).
- `wallet_nonce` = total on-chain tx count at trade time. Low nonce + high-conviction trade = freshly-minted wallet used once.
- `wallet_funded_by_cex` = binary flag: did the wallet's first non-trivial inbound transfer come from a known CEX hot wallet (Kraken, Binance, Coinbase)? Captures the common insider pattern of funding via CEX → single-purpose wallet → trade. Matches the documented Biden-pardons wallet ("shared Kraken wallet" per Columbia paper).
- All three extracted via `scripts/enrich_polygonscan.py` — one `txlist` call per unique wallet. Free API key, ~1h for ~50k wallets at free-tier 5 req/sec.

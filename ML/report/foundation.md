...
| Features | Seven layers, all strictly no-lookahead (every row's feature at time `t` uses only rows with `timestamp < t`, enforced via `groupby().cumsum().shift(1)` and `rolling(window, on='timestamp', closed='left')`). The final consolidated CSV is produced by `scripts/20_finalize_dataset.py`, which physically drops four classes of non-feature / leaky / market-identifying columns so the CSV *is* the modelling feature set (pre-drop snapshot preserved at `data/03_consolidated_dataset.pre_dropped_variables.csv` for audit). Retained feature layers: (1) **trade-local** — `log_size` only. `side` and `outcomeIndex` were dropped under P0-11 (they jointly determine `bet_correct` via a formula whose mapping flips across market resolution types, producing catastrophic test-set inversion on single-resolution cohorts — the exam's test set is all-NO ceasefires); (2) **market context, market-normalised only** — `market_price_vol_last_1h`. `market_buy_share_running` was dropped under P0-12 (running share of BUY is a direction-encoding aggregate with a 30-point train-vs-test cohort shift). Absolute-scale cousins (`market_trade_count_so_far`, `market_volume_so_far_usd`, `market_vol_1h_log`, `market_vol_24h_log`) are dropped under P0-8 to prevent the model memorising market identity; (3) **time** — `pct_time_elapsed` only (derived from `deadline_ts`, the market's advertised deadline parsed directly from the question string — "by February 27, 2026"); authored at market creation and therefore provably known at trade time. Absolute scalars `time_to_settlement_s` and `log_time_to_settlement` are dropped (P0-8) because they scale with each market's deadline distance and similarly leak identity. Two weaker deadline sources are explicitly rejected: `resolution_ts` (post-hoc CLOB lock — leaks future info) and Gamma's `endDate` (stale or batch-placeholder `2026-01-31` on 34 of 74 markets); (4) **wallet global** — `wallet_prior_trades`, `wallet_prior_volume_usd`, `wallet_first_minus_trade_sec`, and the **causal** `wallet_prior_win_rate_causal` (cumulative mean of `bet_correct` restricted to priors with `resolution_ts < t`) with matching indicator `wallet_has_resolved_priors`. The naive `wallet_prior_win_rate` (sum over all priors regardless of resolution) is dropped — it carried a +0.13 leak-driven correlation with the target (see §11 P0-9); (4b) **wallet-in-market** in three remaining clusters after the P0-11 / P0-12 direction-encoding drop: bet-slicing (`wallet_trades_in_market_last_1min/10min/60min`, `wallet_is_burst`, `wallet_median_gap_in_market`), symmetric diversity (`wallet_spread_ratio` — min/max of cum₀/cum₁, which is invariant under swapping the two outcome tokens so it cannot re-open the inversion channel), position size (`wallet_is_whale_in_market` — causal expanding p95, see §11). Dropped: `wallet_directional_purity_in_market`, `wallet_has_both_sides_in_market`, `wallet_position_size_before_trade`, `trade_size_vs_position_pct`, `wallet_cumvol_same_side_last_10min` (all P0-12 — direction-dependent aggregates), `is_position_exit` + `is_position_flip` (P0-2 denominator bug). Depth: `wallet_prior_trades_in_market`; (5) **interactions** — `size_vs_wallet_avg`, `size_vs_market_cumvol_pct`, `size_vs_market_avg`. `size_x_time_to_settlement` is dropped (P0-8 — inherits market identity from `log_time_to_settlement`); (6) **on-chain identity (Layer 6)** — `wallet_polygon_age_at_t_days`, `wallet_polygon_nonce_at_t` (+ log), `wallet_n_inbound_at_t` (+ log), `wallet_n_cex_deposits_at_t`, `wallet_cex_usdc_cumulative_at_t` (+ log), `days_from_first_usdc_to_t`, `wallet_funded_by_cex_scoped` (the **causal** variant: 1 iff the wallet's first CEX inflow happened strictly before the trade), `wallet_enriched`. The unscoped `wallet_funded_by_cex` is dropped as a defence-in-depth removal of a structurally-leaky lifetime flag; (7) **cross-market diversity** — `wallet_market_category_entropy`, Shannon entropy (nats) over coarse category distribution of a wallet's prior distinct markets across the full Polymarket universe, derived from the HuggingFace mirror via `scripts/10_wallet_category_entropy.py`. Five binary missingness indicators (`wallet_has_prior_trades`, `wallet_has_prior_trades_in_market`, `wallet_has_cross_market_history`, `market_timing_known`, `wallet_enriched`, plus the new `wallet_has_resolved_priors`) accompany the features per §5.6. **The market-implied probability (`price` / `market_implied_prob`) is deliberately excluded from the feature set** so `p_hat` is independent of the market's own belief and the gap `p_hat - market_implied_prob` is a clean signal. `market_implied_prob` is retained in the CSV only as the trading-rule benchmark. News proximity (GDELT) remains deferred to future work. |
...
- *Resolved 22 Apr.* Future-looking leakage audit across all 85 feature columns, followed by a second-pass rigour sweep that caught a data-quality issue in the Gamma `endDate` field. Source fixes live in `scripts/01_polymarket_api.py` (the canonical feature definitions); existing-CSV patches by `scripts/17_fix_leakage.py` (whale + end_date), `scripts/18_fix_deadline.py` (deadline_ts + re-derived time features), and Alex's `scripts/16_patch_resolution_ts.py` (robust resolution-lock backfill). A regression guard at `scripts/_check_causal.py` asserts 54 causal invariants (first-in-group = 0 / NaN, per-market monotonicity of cumulative features, per-feature NaN-rate bounds, `time_to_settlement_s == deadline_ts − timestamp`, etc.) and is the canary for future rebuilds.
- **Leak A — `wallet_is_whale_in_market`.** Old definition compared each wallet's running market volume to the 95th percentile of *end-of-market* wallet totals, so the threshold depended on future trades; empirically 0.0 % of non-whales-by-end were ever flagged and 44 % of final-whales were flagged on average across their trades, with 19.9 % of trades in the first decile of market life already flagged. New definition is an expanding per-market p95: at trade *t* in market *m*, the threshold is the 95th percentile of per-wallet running volumes computed from trades with timestamp strictly less than *t*, with a 20-wallet warm-up, implemented via `sortedcontainers.SortedList` for `O(N log W)` per market. Post-patch diagnostic: 0 first-(wallet, market) trades flagged (was 3,624); 19,418 leaky 1-flags removed; 66,595 new flags fire for wallets that dominate early but cool off by market close — a causally correct signal that was masked by the old end-state threshold. Non-whales-by-end now show 0.4 % flag-share (was 0.0 %), confirming the expanding p95 crosses and the flag does not depend on end-state any more.
- **Leak B — time features built from `resolution_ts`.** `time_to_settlement_s`, `log_time_to_settlement`, `pct_time_elapsed`, and `size_x_time_to_settlement` were originally built from the post-hoc CLOB-lock timestamp `resolution_ts`. 95 % of rows used the leaky source and resolution_ts disagreed with `end_date` on 100 % of those rows (quartile deltas −10 to +73 days). Fix 1: swap all four features to `end_date`.
- **Data-quality follow-up — Gamma `endDate` is stale on 34 / 74 markets.** The second-pass sweep caught that `endDate = 2026-01-31` is a placeholder Gamma returns for 34 Feb / Jan strike markets under event 114242, unrelated to the actual market deadlines that appear in the market's own question string. End_date-based time features therefore went negative on ~44 % of rows and `pct_time_elapsed` was 100 % NaN on 20 markets. Fix 2 (`scripts/18_fix_deadline.py`): parse the "by &lt;Month&gt; &lt;Day&gt;, &lt;Year&gt;" deadline from the question (succeeds on all 74 / 74 markets with a year-fallback rule for event-236884 "by March X?"), store it as `deadline_ts`, and re-derive the four time features from it. After fix: negative-time rows drop to 314 k (genuine post-deadline trading), only 1 / 74 markets has 100 % NaN `pct_time_elapsed` (the single ceasefire market whose first recorded trade is after its advertised deadline, 131 rows). `deadline_ts` is the published deadline and provably known at trade time, so using it as a feature source is causal. `settlement_minus_trade_sec` keeps `resolution_ts` upstream because its sole role is the §4 post-resolution filter, which is not a model feature.
- **Resolution_ts backfill (`scripts/16_patch_resolution_ts.py`).** Three strike markets (Feb 8, Feb 27, Mar 3) previously had `resolution_ts = NaT` because the old lock detector rejected valid locks whenever a late outlier trade dipped below `LOCK_UNLOCK_FLOOR`. A robust-median variant of the same heuristic fills them (locks at 2026-02-08 18:53, 2026-02-28 01:56, 2026-02-28 07:04 respectively). After backfill, 74 / 74 markets carry a resolution timestamp, so the §4 pre-modelling filter correctly drops post-resolution trades in those three markets too.
- **Latent structural risk — `wallet_funded_by_cex` (static).** Unscoped variant was at risk of leaking future CEX funding, but empirically 0 rows in the current dataset exhibit static = 1 with scoped = 0. Kept as-is; downstream modelling code should prefer `wallet_funded_by_cex_scoped`.
- *Resolved 22 Apr (late).* **Leak C — `wallet_prior_win_rate` peeks at future market resolutions (Alex's P0-9).** The naive cumulative mean of prior trades' `bet_correct` is well-defined only once those priors' markets have resolved (`resolution_ts`). In our build, all 74 markets had resolved by CSV-write time, so every prior's `bet_correct` is populated — but at any given trade time *t*, the values for priors with `resolution_ts > t` were not observable. Empirically: **83.6 % of rows have ≥ 1 unresolved prior at trade time**; for the typical row with history, 68 % of its "prior trades" were in markets still pending. Pearson r with `bet_correct`: leaky version **+0.3673**, causally-clean version **+0.2357** — a +0.1316 leak-driven component on the strongest linear correlate in the dataset. Fix: `_add_running_wallet_features` in `01_polymarket_api.py` now emits `wallet_prior_win_rate_causal` (cumulative mean over priors with `resolution_ts < t`) plus `wallet_has_resolved_priors` (1 iff ≥ 1 resolved prior exists); existing CSV patched by `scripts/19_add_causal_win_rate.py` (backup `data/03_consolidated_dataset.pre_causal_win_rate.csv`).
- *Resolved 22 Apr (late).* **Physical-drop finalisation (`scripts/20_finalize_dataset.py`).** After all leak fixes landed, the CSV went through a final physical drop pass so it is the modelling feature set rather than "all features + an exclusion list." Five drop tiers, 33 columns total: **Tier 1 (confirmed leak / bug, 4 cols)** — `wallet_prior_win_rate` (replaced by causal), `is_position_exit` + `is_position_flip` (P0-2 denominator bug), `wallet_funded_by_cex` (structurally leaky lifetime flag, `_scoped` retained); **Tier 2 (P0-8 market-identity absolute-scale, 7 cols)** — `time_to_settlement_s`, `log_time_to_settlement`, `market_volume_so_far_usd`, `market_vol_1h_log`, `market_vol_24h_log`, `market_trade_count_so_far`, `size_x_time_to_settlement`; **Tier 3 (metadata bloat, 13 cols)** — `conditionId, title, slug_x, slug_y, icon, eventSlug, outcome, name, pseudonym, bio, profileImage, profileImageOptimized, outcomes`; **Tier 4 (obsolete, 1 col)** — `split`; **Tier 5 (direction-encoding P0-11 / P0-12, 8 cols)** — `side`, `outcomeIndex` (P0-11: the pair perfectly determines `bet_correct` via `(outcomeIndex == winning_outcome_index) == (side == BUY)`, and the mapping flips between YES- and NO-resolved markets so a mixed training cohort learns an averaged rule that inverts on a single-resolution test cohort — Alex observed ROC 0.00-0.04 on the all-NO ceasefire test), `wallet_position_size_before_trade`, `trade_size_vs_position_pct`, `wallet_cumvol_same_side_last_10min`, `wallet_directional_purity_in_market`, `wallet_has_both_sides_in_market`, `market_buy_share_running` (P0-12: the same direction-encoding channel routed through signed positions, same-side filters, and outcomeIndex aggregates). CSV went from 87 → 54 cols, 1.16 GB → 819 MB. Pre-drop state preserved at `data/03_consolidated_dataset.pre_dropped_variables.csv` for audit and sensitivity analysis. The regression guard `_check_causal.py` was updated to the new column set (34 / 34 passing).
- **Known limitation — `market_implied_prob` has mixed semantics across sources.** 100 % of HF rows fall back to the trade's execution price (no CLOB history pulled for events 114242 and 236884); only 3 % of API rows match `price` (the rest use CLOB mid-history). Not a feature leak (§4 excludes this column from the feature set), but the economic-benchmark interpretation of the trading-rule gap `p_hat − market_implied_prob` degenerates to `p_hat − execution_price` on HF rows. Fetching CLOB history for the 67 HF markets (~1 h network) is the rigorous fix; if time-constrained, the trading-rule section should note this in one sentence.
...
└── scripts/                                 # Data pipeline + modelling code.
    ...
    ├── 16_patch_resolution_ts.py            # One-shot: robust-median backfill of resolution_ts for 3 strike markets hit by the old lock detector (obsolete after next rebuild).
    ├── 17_fix_leakage.py                    # One-shot: causal rewrite of wallet_is_whale_in_market + end_date-only time features (obsolete after next rebuild; leak fix lives in 01_polymarket_api.py).
    ├── 18_fix_deadline.py                   # One-shot: parse `deadline_ts` from each market's question, rederive time features from it; fixes Gamma endDate placeholder on 34 / 74 markets (obsolete after next rebuild; parser lives in 01_polymarket_api.py:parse_deadline_from_question).
    ├── 19_add_causal_win_rate.py            # One-shot: add wallet_prior_win_rate_causal + wallet_has_resolved_priors; fixes Alex's P0-9 future-info peek in the naive win rate (obsolete after next rebuild; causal logic lives in 01_polymarket_api.py:_add_running_wallet_features).
    ├── 20_finalize_dataset.py               # Physical-drop step: removes 25 columns (leak/bug, market-identity, metadata bloat, obsolete split) from the CSV so it *is* the modelling feature set. Pre-drop snapshot at data/03_consolidated_dataset.pre_dropped_variables.csv.
    └── _check_causal.py                     # Regression guard: 39 causal invariants over the finalised CSV. Run after any rebuild; non-zero exit on regression.
```

...
META_AND_RAW = {
    ...
    "market_trades_1h",
    "wallet_prior_trades",
    # Layer 6 raw that has log variants already in the feature set
    "wallet_polygon_nonce_at_t",
    "wallet_n_inbound_at_t",
    "wallet_cex_usdc_cumulative_at_t",
    # Lifetime binary — small leak risk; keep only the scoped variant
    "wallet_funded_by_cex",
    # Market-identifying absolute-scale features — dropped because with only
    # 7 markets the model otherwise memorises per-market fingerprints (train
    # loss → 0.005 in 1 epoch on v2). Keeping market-agnostic replacements
    # like pct_time_elapsed and the bounded price-state features.
    "time_to_settlement_s",
    "log_time_to_settlement",
    "market_cumvol_log",
    "market_cumtrades",
    "market_cumtrades_log",
    "market_vol_1h_log",
    "market_vol_24h_log",
    "market_trades_1h_log",
}
